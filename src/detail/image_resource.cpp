...
image_resource::image_resource(const image_desc &desc) : desc_{desc} {
  ...
  case image_type::image_1d_array:
    tex_ =
        gl::create_texture_2d(GL_TEXTURE_1D_ARRAY, desc_.format, desc_.size[0],
                              desc_.size[1], desc_.num_mipmaps);
    break;
  case image_type::image_2d:
    tex_ = gl::create_texture_2d(GL_TEXTURE_2D, desc_.format, desc_.size[0],
                                 desc_.size[1], desc_.num_mipmaps);
    break;
  case image_type::image_2d_array:
    tex_ =
        gl::create_texture_3d(GL_TEXTURE_2D_ARRAY, desc_.format, desc_.size[0],
                              desc_.size[1], desc_.size[2], desc_.num_mipmaps);
    break;
  case image_type::image_3d:
    tex_ =
        gl::create_texture_3d(GL_TEXTURE_3D, desc_.format, desc_.size[0],
                              desc_.size[1], desc_.size[2], desc_.num_mipmaps);
    break;
  case image_type::image_cube_map:
    tex_ =
        gl::create_texture_2d(GL_TEXTURE_CUBE_MAP, desc_.format, desc_.size[0],
                              desc_.size[1], desc_.num_mipmaps);
    break;
  case image_type::image_2d_multisample:
    tex_ = gl::create_texture_2d_multisample(GL_TEXTURE_2D_MULTISAMPLE,
                                             desc_.format, desc_.size[0],
                                             desc_.size[1], desc_.num_samples);
    break;
  case image_type::image_2d_multisample_array:
    tex_ = gl::create_texture_3d_multisample(
