...
void* staging_buffer::try_allocate(
...
void* staging_pool::get_staging_buffer(vk::Device device, VmaAllocator allocator,
    ...
    for (auto& b : buffers_) {
        if (auto ptr = b.try_allocate(align, size, out_buffer, out_offset)) { return ptr; }
    ...
    if (!buffers.fetch_if(sb, [&](const staging_buffer& sb) { return sb.space >= align + size; })) {
        ...
        const VmaAllocationCreateInfo alloc_create_info{.flags = VMA_ALLOCATION_CREATE_MAPPED_BIT,
        ...
        const vk::BufferCreateInfo buffer_create_info{
            .size = alloc_size,
                ...
                .sharingMode = vk::SharingMode::eExclusive  // TODO async
        };

        VkBuffer vk_buffer;
        if (auto result = vmaCreateBuffer(allocator,
                    &static_cast<const VkBufferCreateInfo&>(buffer_create_info), &alloc_create_info,
                    &vk_buffer, &sb.allocation, &sb.allocation_info);
                result != VK_SUCCESS) {
        ...
        // map the buffer
        sb.base = sb.allocation_info.pMappedData;
        sb.ptr = sb.base;
