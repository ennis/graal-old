...
constexpr struct empty_t { constexpr empty_t() noexcept = default; } empty;

template <class E> class flags {
public:
  static_assert(is_flags<E>::value, "flags::flags is disallowed for this type; "
...
private:
  ...
  using convertible =
      std::enable_if<std::is_convertible<T, enum_type>::value, Res>;
...
public:
  ...
  flags(FwIter b, FwIter e,
        typename convertible<decltype(*b)>::type =
  ...
  constexpr explicit
  operator std::bitset<flags<E>::bit_size()>() const noexcept {
    return to_bitset();
  }
  ...
  template <class FwIter>
  auto insert(FwIter i1, FwIter i2) noexcept(
      noexcept(++i1) &&noexcept(*i1) &&noexcept(i1 == i2)) ->
...
template <class E> void swap(flags<E> &fl1, flags<E> &fl2) noexcept {
  fl1.swap(fl2);
}
